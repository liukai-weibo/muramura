...
- 快速捕获想法，点击「开始记录」即建立「进行中」事项并直接进入执行。
- 事项推进主流程：「进行中」→「已复盘」；「想试试 / 以后再说 / 暂停 / 等待复盘 / 放弃 / 已归档未复盘」为历史兼容状态，仅供历史数据与旧客户兼容，不作为新增事项的操作入口（新捕获直接进入进行中）。
- 支持形成、验证、修订及复用方法，并保留版本和证据关系。
- 提供全局搜索、周期 Dashboard、状态历史和统一回收站；大项可归档/取消归档，归档级联且归档大项不可继续记录。
- 提供手记（今日小记）按日期编辑、历史查看、自动保存和 Markdown 整理；支持全日期情绪与一日三餐的记录、统计与日历浏览。

**AI 工作台**

- 提供 H5 AI 对话，支持 SSE 流式回复、分块增量 Markdown 解析、Worker 异步处理和长消息虚拟列表；支持停止、异常清理、会话隔离、刷新恢复与稳定滚动（微信小程序暂不作为桌宠与流式优化验收目标）。
- 圈圈 AI 可按当前用户读取全部可用历史手记、当天手记、事项状态、全日期情绪与三餐以及工作台摘要；AI 会话与业务数据按用户隔离。
- AI 行动建议按「近期回报」取舍：先比较探索主线近 30 天回报信号（近30天执行/复盘/派生方法应用/最近活动），再比较主线内事项回报，无主线则直接事项级比较；服务端只注入真实聚合信号，AI 不编造回报率/百分比。
- AI 时间感知：注入本地真实时间、记录时刻上屏、基于时刻推理。
- 首页提供「近期状态小结」卡片：AI 基于近期手记/事项/复盘自动总结当前状态、阻力与建议，可点开手动重新生成；数据落地 `daily_summaries`。
- 首页提供「今日饮食推荐」卡片：AI 先分析最近 7 天的饮食/手记/情绪，给出 1-2 条只针对饮食的建议（具体到食物/怎么吃/补水/避免什么），与整体状态小结明确分工；数据落地 `daily_diet_recommendations`。今日饮食的提示词为「顶级营养师」版（按当前时间分析今日剩余饮食、推荐简单好记、结尾无问句钩子、结论不重复基础信息）。
- 首页 AI 卡片生成优化：打开页面自动生成、点击重新生成后在后台持续生成（关闭弹窗不中断），卡片与弹窗共享流式草稿预览；服务端提供 ephemeral 轻量生成模式提高首字速度。
- 今日饮食推荐「个人档案」可配置（身高/体重/年龄/性别/目标/日常活动量/健康状态，留空项 AI 不编造），并默认偏好在周边新零售超市常见的食材；2026-08-30 起个人档案改为服务端单行存储 + 安全审计（module=daily_diet / action=update，绑定当前用户，支持同账号多端同步），数据落地 `user_diet_profiles`。
- 首页支持自定义 AI 卡片：用户可创建/隐藏自定义 AI 卡片（标题、提示指令、尺寸/主题/刷新模式），每日缓存刷新并记录可读卡片标题的「刷新」审计。
- H5 AI 页面提供 Rive 桌宠及 SVG 降级、思考态、欢迎态和基于时间/星期/节日的本地气泡文案；桌宠交互不写入消息或数据库。

**账户、权限与安全**

- 支持注册、登录、退出、Cookie 会话和会话过期；账户页支持修改密码（成功后撤销全部会话，并要求用新密码重新登录）。
- 单用户数据隔离：全部业务数据按当前会话用户隔离，跨用户资源统一不可见。
- 所有用户固定拥有 `member`；`platform_admin` 是可选附加角色。平台管理员可查看脱敏用户列表、授予/撤销他人的管理员角色、撤销他人的全部会话，但不能绕过 owner scope，也不能查看密码、密码哈希、Cookie、Token 或会话秘密。
- 安全审计中心：按用户、功能模块、时间、操作类型与关键词查看内容变更与操作日志并导出 CSV；审计快照以中文可读展示，点击可查看完整内容弹窗，复盘/小结等 AI 生成类内容支持 Markdown 渲染；模块与动作按封板枚举约束并随需求扩展。

**数据与备份**

- 提供完整 JSON 备份导出与安全恢复，并保持当前备份版本兼容。
- 数据可恢复、业务关系可追溯，强调账户间数据隔离。

**桌面端（Tauri Windows）**

- 提供 Tauri Windows 桌面客户端，桌面上承载工作台并通过本机 API（`http://127.0.0.1:32146`）使用全部能力。
- 「我的-账户」提供【检查更新】：Tauri updater 连接 GitHub Release，展示当前/最新版本、下载进度并自动安装重启。
- 桌面更新源为仓库根 `updater.json`（GitHub Release + gh-proxy 实时代理），构建签名私钥仅存本机/CI、不进入镜像与仓库。

## 当前状态

- 在线账户与单用户数据隔离 V0 已完成产品验收、Git 归档并封板。
- 平台角色与最小权限管理 V1 已完成产品验收、真实运行接入、Git 归档并封板。
- 当前源码要求 MySQL Schema 至少为版本 31；实际运行版本以 API `/health` 返回值为准。今日小记及其 AI 专属会话依赖 Migration 017–018；每日状态小结依赖 Migration 023；今日饮食推荐依赖 Migration 024；安全审计中心依赖 Migration 025（含 Schema 25），二期枚举扩展依赖 Migration 027（含 Schema 27）；首页自定义 AI 卡片依赖 Migration 026（含 Schema 26）；AI 时间感知与长期探索归档依赖 Migration 028，审计刷新动作依赖 Migration 029；今日饮食推荐个人档案依赖 Migration 031。云 / UAT 环境未完成对应迁移前不应宣称对应能力可用。
...
- 数据库：MySQL 8.4，Migration 001–031；当前运行版本以 API `/health` 为准。
...
- 全部业务数据的多端/云端同步，以及安卓客户端；当前仅今日饮食推荐个人档案支持同账号多端同步。
