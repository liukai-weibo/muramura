...
describe.runIf(enabled)('API Schema 7 startup gate', () => {
  beforeAll(async () => {
  ...
  it('does not create or listen on Schema 6 and allows the listen step only on Schema 7', async () => {
    const [rows] = await app.query<Array<RowDataPacket & { version: number; name: string; checksum: string; applied_at: Date }>>('SELECT version,name,checksum,applied_at FROM schema_migrations WHERE version=7')
    ...
    await expect(startApiMain(environment(), { createServer, listen, log })).rejects.toMatchObject({
      details: {
        reason: 'schema-version-behind', database, actualSchemaVersion: 6, requiredSchemaVersion: 7,
      },
    })
    expect(createServer).not.toHaveBeenCalled(); expect(listen).not.toHaveBeenCalled(); expect(log).not.toHaveBeenCalled()
    const schema6Server = createApiServer({ host: process.env.MYSQL_HOST!, port: Number(process.env.MYSQL_PORT!), database, user: appUser, password: appPassword, connectionLimit: 1 })
    await new Promise<void>(resolve => schema6Server.listen(0, '127.0.0.1', resolve))
    const health = await new Promise<{ status: number; body: string }>((resolve, reject) => {
      const request = http.get({ host: '127.0.0.1', port: (schema6Server.address() as { port: number }).port, path: '/health' }, response => {
    ...
    await new Promise<void>(resolve => schema6Server.close(() => resolve()))
    const row = rows[0]!
