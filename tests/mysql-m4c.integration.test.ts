...
describe.runIf(enabled)('MySQL M4-C derived Review workflow and BackupData', () => {
  ...
  it('round trips complete M4 data through existing BackupData without metadata leakage', async () => {
    ...
    const repository = new MySqlBackupRepository(app); const service = new BackupApplicationService(repository); const key = id(); await app.execute('INSERT INTO system_metadata(`key`,value,created_at,updated_at) VALUES(?,?,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [key, 'private'])
