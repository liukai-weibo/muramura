...
describe.runIf(enabled)('platform administration API', () => {
  ...
  it('lists fixed pages with strict query validation, stable DTOs and literal search', async () => {
    ...
    expect(Object.keys(first.body.items[0])).toEqual(['id', 'username', 'roles', 'createdAt', 'deletedAt'])
  ...
  it('changes only another user role, returns a real reread, and maps protected failures', async () => {
    ...
    expect(granted.status).toBe(200); expect(granted.body).toEqual({ id: targetId, username: 'literal%_=target', roles: ['member', 'platform_admin'], createdAt: expect.any(String), deletedAt: null })
  ...
  it('soft-deletes, reads and restores another account without reviving old access', async () => {
    ...
    expectError(await json(`/api/v1/admin/users/${adminId}/soft-delete`, { operationId: crypto.randomUUID() }, adminCookie), 403, 'FORBIDDEN')
    const operationId = crypto.randomUUID()
    const deleted = await json(`/api/v1/admin/users/${targetId}/soft-delete`, { operationId }, adminCookie)
    expect(deleted.status).toBe(200)
    expect(deleted.body).toMatchObject({ id: targetId, roles: ['member'], deletedAt: expect.any(String) })
    expectError(await get('/api/v1/auth/session', oldCookie), 401, 'UNAUTHORIZED')
    expectError(await json('/api/v1/auth/login', { username: 'literal%_=target', password: 'password-target' }), 401, 'UNAUTHORIZED')
    const reread = await get(`/api/v1/admin/users/${targetId}`, adminCookie)
    expect(reread.status).toBe(200)
    expect(reread.body).toEqual(deleted.body)
    expectError(await json(`/api/v1/admin/users/${targetId}/roles`, { roles: ['member', 'platform_admin'], operationId: crypto.randomUUID() }, adminCookie, 'PUT'), 409, 'CONFLICT')
    expectError(await json(`/api/v1/admin/users/${targetId}/soft-delete`, { operationId }, adminCookie), 409, 'CONFLICT')

    const restored = await json(`/api/v1/admin/users/${targetId}/restore`, { operationId: crypto.randomUUID() }, adminCookie)
    expect(restored.status).toBe(200)
    expect(restored.body).toMatchObject({ id: targetId, roles: ['member'], deletedAt: null })
    expectError(await get('/api/v1/auth/session', oldCookie), 401, 'UNAUTHORIZED')
    const freshLogin = await json('/api/v1/auth/login', { username: 'literal%_=target', password: 'password-target' })
    expect(freshLogin.status).toBe(200)
    expect(freshLogin.body.user.roles).toEqual(['member'])
  })
})
