...
describe.runIf(enabled)('MySQL M3-B method application and purge repositories', () => {
  ...
  it('purges a trashed Method only with proof, preserving historical Evidence and Application atomically', async () => {
    ...
    const invalidMethod = await activeMethod(); const invalidItem = await new MySqlItemRepository(app).create({ title: '异常应用' }); await app.query('INSERT INTO method_applications(id,method_id,method_version,item_id,created_at,updated_at) VALUES(?,?,99,?,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [id(), invalidMethod.id, invalidItem.id]); await repository.moveToTrash(invalidMethod.id); const before = await snapshot()
  ...
  it('purges a deleted Item through real relationships and rolls every affected table back on terminal failure', async () => {
    ...
    await app.query('INSERT INTO item_links(id,source_review_id,target_item_id,type,created_at,updated_at) VALUES(?,?,?,"derived_from_review",UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [id(), sourceReview.id, sourceItem.id]); await items.delete(sourceItem.id); await items.purgeDeletedBefore('2999-01-01T00:00:00.000Z')
