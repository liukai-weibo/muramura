...
describe.runIf(mysqlIntegrationEnabled)('MySQL M1 repeatable integration verification', () => {
  it('replays current migrations idempotently and rejects checksum drift in a temporary database', async () => {
    await withTemporaryDatabase(async ({ migrator, migrationDirectory }) => {
      ...
      const normalizedLatestMigration = fs.readFileSync(latestMigrationPath, 'utf8').replace(/\r\n?/g, '\n')
      const legacyCrlfChecksum = crypto.createHash('sha256').update(normalizedLatestMigration.replace(/\n/g, '\r\n')).digest('hex')
      expect(legacyCrlfChecksum).not.toBe(expected.at(-1)!.checksum)
      await migrator.query('UPDATE schema_migrations SET checksum = ? WHERE version = ?', [legacyCrlfChecksum, expected.at(-1)!.version])
