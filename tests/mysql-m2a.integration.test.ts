...
describe.runIf(enabled)('MySQL M2-A Item Repository', () => {
  ...
  it('purges unassociated Items and their determined links without treating isolated tombstones as blockers', async () => {
    ...
    await appPool!.execute('INSERT INTO item_links(id,source_review_id,target_item_id,type,created_at,updated_at) VALUES(?,?,?,"derived",UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [testId(), itemReview, target.id])
    await appPool!.execute('INSERT INTO item_links(id,source_review_id,target_item_id,type,created_at,updated_at) VALUES(?,?,?,"derived",UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [testId(), otherReview, item.id])
    const tombstoneId = testId()
    await appPool!.execute('INSERT INTO method_tombstones(method_id,title,permanently_deleted_at,versions,created_at,updated_at) VALUES(?,"isolated",UTC_TIMESTAMP(3),JSON_ARRAY(),UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [tombstoneId])
    await repository.delete(item.id)
    await repository.purgeDeletedBefore(cutoff)
  ...
  it('purges an Item with a real MethodApplication relation and removes now-unreferenced Method history', async () => {
    ...
    await appPool!.execute('INSERT INTO method_versions(id,method_id,version,title,applicable,unsuitable,steps,source_review_id,created_at,updated_at) VALUES(?,?,1,"应用方法","","","",NULL,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [versionId, methodId])
    await appPool!.execute('INSERT INTO method_applications(id,method_id,method_version,item_id,created_at,updated_at) VALUES(?,?,1,?,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [applicationId, methodId, item.id])
    await appPool!.execute('INSERT INTO method_tombstones(method_id,title,permanently_deleted_at,versions,created_at,updated_at) VALUES(?,?,UTC_TIMESTAMP(3),JSON_ARRAY(JSON_OBJECT("version", 1)),UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [tombstoneId, '应用方法'])
    const linkedTarget = await repository.create({ title: testId() })
    await appPool!.execute('INSERT INTO item_links(id,source_review_id,target_item_id,type,created_at,updated_at) VALUES(?,?,?,"derived",UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [testId(), reviewId, linkedTarget.id])
    await repository.delete(item.id)
  ...
  it('purges an Item with a real MethodEvidence relation without orphaning Method history', async () => {
    ...
    await appPool!.execute('INSERT INTO method_versions(id,method_id,version,title,applicable,unsuitable,steps,source_review_id,created_at,updated_at) VALUES(?,?,1,"证据方法","","","",NULL,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [versionId, methodId])
    await appPool!.execute('INSERT INTO method_evidence(id,method_id,review_id,relation,method_version,created_at,updated_at) VALUES(?,?,?,"formation",1,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [evidenceId, methodId, reviewId])
    const target = await repository.create({ title: testId() })
    await appPool!.execute('INSERT INTO item_links(id,source_review_id,target_item_id,type,created_at,updated_at) VALUES(?,?,?,"derived",UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [testId(), reviewId, target.id])
    await repository.delete(item.id)
  ...
  it('nulls only the purged Review source on a Method with remaining real Evidence', async () => {
    ...
    await appPool!.execute('INSERT INTO method_versions(id,method_id,version,title,applicable,unsuitable,steps,source_review_id,created_at,updated_at) VALUES(?,?,1,"保留方法","","","",?,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [deletedSourceVersionId, methodId, deletedReviewId])
    await appPool!.execute('INSERT INTO method_versions(id,method_id,version,title,applicable,unsuitable,steps,source_review_id,created_at,updated_at) VALUES(?,?,2,"保留方法","","","",?,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [retainedVersionId, methodId, retainedReviewId])
    await appPool!.execute('INSERT INTO method_evidence(id,method_id,review_id,relation,method_version,created_at,updated_at) VALUES(?,?,?,"validation",2,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [retainedEvidenceId, methodId, retainedReviewId])
    await appPool!.execute('INSERT INTO item_links(id,source_review_id,target_item_id,type,created_at,updated_at) VALUES(?,?,?,"derived",UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))', [testId(), deletedReviewId, retainedItem.id])
    await repository.delete(item.id)
