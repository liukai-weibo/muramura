...
function createMigrationDirectory(): string {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'knowledge-base-diet-'))
...
async function withTemporaryDatabase(work: (pools: { app: ReturnType<typeof createMySqlPool>; migrator: ReturnType<typeof createMySqlPool> }) => Promise<void>): Promise<void> {
  ...
  const database = 'kb_diet_' + suffix
  const appUser = 'kbdiet_' + suffix.slice(0, 20)
...
describe.runIf(mysqlIntegrationEnabled)('daily diet MySQL repository', () => {
  afterAll(() => undefined)
  ...
  it('upserts one row per date and scopes to owner', { timeout: 30_000 }, async () => {
    await withTemporaryDatabase(async ({ app, migrator }) => {
      ...
      const repository = new MySqlDailyDietRecommendationRepository(app, createScope('owner-a'))
  ...
  it('lists by date range and keeps other owners isolated', { timeout: 30_000 }, async () => {
    await withTemporaryDatabase(async ({ app, migrator }) => {
      ...
      const repositoryA = new MySqlDailyDietRecommendationRepository(app, createScope('owner-a'))
      const repositoryB = new MySqlDailyDietRecommendationRepository(app, createScope('owner-b'))
