...
const toWaitingReview = async (bundle: ReturnType<typeof open>, title = '待复盘事项') => {
  ...
  await bundle.itemRepository.changeStatus(item.id, 'waiting_review')
  return item
}
...
describe('SQLite S4 review workflow, search, and dashboard candidate repositories', () => {
  it('completes review with formation, derived item, link, reviewed event, and one transaction', async () => {
    ...
    expect(await bundle.itemRepository.listStatusEvents(item.id)).toMatchObject([{ toStatus: 'idea_to_try' }, { toStatus: 'idea_later' }, { toStatus: 'idea_to_try' }, { toStatus: 'doing' }, { toStatus: 'waiting_review' }, { fromStatus: 'waiting_review', toStatus: 'reviewed' }])
  ...
  it('rejects non-waiting, deleted, duplicate review, invalid method selection, and conflicting method choices without writes', async () => {
    ...
    const before = await bundle.backupRepository.exportData()
    await expect(bundle.reviewWorkflowRepository.complete(input(nonWaiting.id))).rejects.toThrow('只有待复盘事项可以完成复盘')
    expect(await bundle.backupRepository.exportData()).toEqual(before)
    ...
    await expect(bundle.reviewWorkflowRepository.complete(input(complete.id))).rejects.toThrow('只有待复盘事项可以完成复盘')
