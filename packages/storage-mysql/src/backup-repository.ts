  ...
  async replaceData(data: BackupData | BackupDataV3): Promise<void> {
    ...
    await runInMySqlTransaction(this.pool, async connection => {
      ...
      for (const event of v3Data.itemStatusEvents) {
        ...
        await connection.execute(owner ? 'INSERT INTO item_status_events(id,item_id,from_status,to_status,created_at,owner_user_id) VALUES(?,?,?,?,?,?)' : 'INSERT INTO item_status_events(id,item_id,from_status,to_status,created_at) VALUES(?,?,?,?,?)', owner ? [event.id, event.itemId, event.fromStatus ?? null, event.toStatus, mysqlDateTime(event.createdAt), owner] : [event.id, event.itemId, event.fromStatus ?? null, event.toStatus, mysqlDateTime(event.createdAt)])
      }
    })
...
async function insertExplorationTracks(connection: PoolConnection, entries: BackupExplorationTrack[], owner?: string) {
  for (const entry of entries) await connection.execute(owner ? 'INSERT INTO exploration_tracks(id,name,normalized_name,created_at,updated_at,deleted_at,owner_user_id) VALUES(?,?,?,?,?,?,?)' : 'INSERT INTO exploration_tracks(id,name,normalized_name,created_at,updated_at,deleted_at) VALUES(?,?,?,?,?,?)', owner ? [entry.id, entry.name, entry.normalizedName, mysqlDateTime(entry.createdAt), mysqlDateTime(entry.updatedAt), entry.deletedAt ? mysqlDateTime(entry.deletedAt) : null, owner] : [entry.id, entry.name, entry.normalizedName, mysqlDateTime(entry.createdAt), mysqlDateTime(entry.updatedAt), entry.deletedAt ? mysqlDateTime(entry.deletedAt) : null])
}
async function insertItems(connection: PoolConnection, entries: Item[], owner?: string) {
  for (const entry of entries) await connection.execute(owner ? 'INSERT INTO items(id,title,content,status,start_action,exploration_track_id,created_at,updated_at,deleted_at,owner_user_id) VALUES(?,?,?,?,?,?,?,?,?,?)' : 'INSERT INTO items(id,title,content,status,start_action,exploration_track_id,created_at,updated_at,deleted_at) VALUES(?,?,?,?,?,?,?,?,?)', owner ? [entry.id, entry.title, entry.content, entry.status, entry.startAction ?? null, entry.explorationTrackId ?? null, mysqlDateTime(entry.createdAt), mysqlDateTime(entry.updatedAt), entry.deletedAt ? mysqlDateTime(entry.deletedAt) : null, owner] : [entry.id, entry.title, entry.content, entry.status, entry.startAction ?? null, entry.explorationTrackId ?? null, mysqlDateTime(entry.createdAt), mysqlDateTime(entry.updatedAt), entry.deletedAt ? mysqlDateTime(entry.deletedAt) : null])
}
...
async function insertVersions(connection: PoolConnection, entries: MethodVersion[], owner?: string) {
  for (const entry of entries) await connection.execute(owner ? 'INSERT INTO method_versions(id,method_id,version,title,applicable,unsuitable,steps,source_review_id,created_at,owner_user_id) VALUES(?,?,?,?,?,?,?,?,?,?)' : 'INSERT INTO method_versions(id,method_id,version,title,applicable,unsuitable,steps,source_review_id,created_at) VALUES(?,?,?,?,?,?,?,?,?)', owner ? [entry.id, entry.methodId, entry.version, entry.title, entry.applicable, entry.unsuitable, entry.steps, entry.sourceReviewId ?? null, mysqlDateTime(entry.createdAt), owner] : [entry.id, entry.methodId, entry.version, entry.title, entry.applicable, entry.unsuitable, entry.steps, entry.sourceReviewId ?? null, mysqlDateTime(entry.createdAt)])
}
async function insertEvidence(connection: PoolConnection, entries: MethodEvidence[], owner?: string) {
  for (const entry of entries) await connection.execute(owner ? 'INSERT INTO method_evidence(id,method_id,review_id,relation,method_version,created_at,owner_user_id) VALUES(?,?,?,?,?,?,?)' : 'INSERT INTO method_evidence(id,method_id,review_id,relation,method_version,created_at) VALUES(?,?,?,?,?,?)', owner ? [entry.id, entry.methodId, entry.reviewId, entry.relation ?? 'unknown', entry.methodVersion ?? null, mysqlDateTime(entry.createdAt), owner] : [entry.id, entry.methodId, entry.reviewId, entry.relation ?? 'unknown', entry.methodVersion ?? null, mysqlDateTime(entry.createdAt)])
}
async function insertApplications(connection: PoolConnection, entries: MethodApplication[], owner?: string) {
  for (const entry of entries) await connection.execute(owner ? 'INSERT INTO method_applications(id,method_id,method_version,item_id,created_at,owner_user_id) VALUES(?,?,?,?,?,?)' : 'INSERT INTO method_applications(id,method_id,method_version,item_id,created_at) VALUES(?,?,?,?,?)', owner ? [entry.id, entry.methodId, entry.methodVersion, entry.itemId, mysqlDateTime(entry.createdAt), owner] : [entry.id, entry.methodId, entry.methodVersion, entry.itemId, mysqlDateTime(entry.createdAt)])
}
async function insertTombstones(connection: PoolConnection, entries: MethodTombstone[], owner?: string) {
  for (const entry of entries) await connection.execute(owner ? 'INSERT INTO method_tombstones(method_id,title,permanently_deleted_at,versions,owner_user_id) VALUES(?,?,?,?,?)' : 'INSERT INTO method_tombstones(method_id,title,permanently_deleted_at,versions) VALUES(?,?,?,?)', owner ? [entry.methodId, entry.title, mysqlDateTime(entry.permanentlyDeletedAt), JSON.stringify(entry.versions), owner] : [entry.methodId, entry.title, mysqlDateTime(entry.permanentlyDeletedAt), JSON.stringify(entry.versions)])
}
async function insertLinks(connection: PoolConnection, entries: ItemLink[], owner?: string) {
  for (const entry of entries) await connection.execute(owner ? 'INSERT INTO item_links(id,source_review_id,target_item_id,type,created_at,owner_user_id) VALUES(?,?,?,?,?,?)' : 'INSERT INTO item_links(id,source_review_id,target_item_id,type,created_at) VALUES(?,?,?,?,?)', owner ? [entry.id, entry.sourceReviewId, entry.targetItemId, entry.type, mysqlDateTime(entry.createdAt), owner] : [entry.id, entry.sourceReviewId, entry.targetItemId, entry.type, mysqlDateTime(entry.createdAt)])
}
