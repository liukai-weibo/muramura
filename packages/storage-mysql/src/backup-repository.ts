...
async function insertExplorationTracks(connection: PoolConnection, entries: BackupExplorationTrack[], owner?: string) {
  for (const entry of entries) await connection.execute(owner ? 'INSERT INTO exploration_tracks(id,name,description,normalized_name,created_at,updated_at,deleted_at,owner_user_id) VALUES(?,?,?,?,?,?,?,?)' : 'INSERT INTO exploration_tracks(id,name,description,normalized_name,created_at,updated_at,deleted_at) VALUES(?,?,?,?,?,?,?)', owner ? [entry.id, entry.name, entry.description ?? '', entry.normalizedName, mysqlDateTime(entry.createdAt), mysqlDateTime(entry.updatedAt), entry.deletedAt ? mysqlDateTime(entry.deletedAt) : null, owner] : [entry.id, entry.name, entry.description ?? '', entry.normalizedName, mysqlDateTime(entry.createdAt), mysqlDateTime(entry.updatedAt), entry.deletedAt ? mysqlDateTime(entry.deletedAt) : null])
}
async function insertItems(connection: PoolConnection, entries: Item[], owner?: string) {
  for (const entry of entries) await connection.execute(owner ? 'INSERT INTO items(id,title,content,status,start_action,exploration_track_id,created_at,updated_at,deleted_at,exploration_track_cascade_deleted_at,owner_user_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)' : 'INSERT INTO items(id,title,content,status,start_action,exploration_track_id,created_at,updated_at,deleted_at,exploration_track_cascade_deleted_at) VALUES(?,?,?,?,?,?,?,?,?,?)', owner ? [entry.id, entry.title, entry.content, entry.status, entry.startAction ?? null, entry.explorationTrackId ?? null, mysqlDateTime(entry.createdAt), mysqlDateTime(entry.updatedAt), entry.deletedAt ? mysqlDateTime(entry.deletedAt) : null, entry.explorationTrackCascadeDeletedAt ? mysqlDateTime(entry.explorationTrackCascadeDeletedAt) : null, owner] : [entry.id, entry.title, entry.content, entry.status, entry.startAction ?? null, entry.explorationTrackId ?? null, mysqlDateTime(entry.createdAt), mysqlDateTime(entry.updatedAt), entry.deletedAt ? mysqlDateTime(entry.deletedAt) : null, entry.explorationTrackCascadeDeletedAt ? mysqlDateTime(entry.explorationTrackCascadeDeletedAt) : null])
}
...
const mapItem = (row: RowDataPacket): Item => ({ id: row.id, title: row.title, content: row.content, status: row.status, createdAt: iso(row.created_at), updatedAt: iso(row.updated_at), ...(row.deleted_at == null ? {} : { deletedAt: iso(row.deleted_at) }), ...(row.exploration_track_cascade_deleted_at == null ? {} : { explorationTrackCascadeDeletedAt: iso(row.exploration_track_cascade_deleted_at) }), ...(row.start_action == null ? {} : { startAction: row.start_action }), ...(row.exploration_track_id == null ? {} : { explorationTrackId: row.exploration_track_id }) })
const mapExplorationTrack = (row: RowDataPacket): BackupExplorationTrack => ({ id: row.id, name: row.name, description: row.description ?? '', normalizedName: row.normalized_name, createdAt: iso(row.created_at), updatedAt: iso(row.updated_at), ...(row.deleted_at == null ? {} : { deletedAt: iso(row.deleted_at) }) })
