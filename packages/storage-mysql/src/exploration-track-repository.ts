...
export class MySqlExplorationTrackRepository implements ExplorationTrackRepository, ExplorationTrackWorkflowRepository {
  ...
  async getById(id: string): Promise<ExplorationTrack | undefined> {
    const [rows] = await this.pool.query<TrackRow[]>(this.scope ? 'SELECT * FROM exploration_tracks WHERE id=? AND owner_user_id=?' : 'SELECT * FROM exploration_tracks WHERE id=?', this.scope ? [id,this.scope.userId] : [id])
  ...
  async listActive(): Promise<ExplorationTrackListEntry[]> {
    const [tracks] = await this.pool.query<TrackRow[]>(this.scope ? 'SELECT * FROM exploration_tracks WHERE deleted_at IS NULL AND owner_user_id=? ORDER BY updated_at DESC,id ASC' : 'SELECT * FROM exploration_tracks WHERE deleted_at IS NULL ORDER BY updated_at DESC,id ASC', this.scope ? [this.scope.userId] : [])
    const result: ExplorationTrackListEntry[] = []
    for (const track of tracks) {
      const [items] = await this.pool.query<ItemRow[]>(this.scope ? 'SELECT * FROM items WHERE exploration_track_id=? AND deleted_at IS NULL AND owner_user_id=? ORDER BY created_at DESC,id ASC LIMIT 1' : 'SELECT * FROM items WHERE exploration_track_id=? AND deleted_at IS NULL ORDER BY created_at DESC,id ASC LIMIT 1', this.scope ? [track.id, this.scope.userId] : [track.id])
  ...
  async listSelectable(): Promise<ExplorationTrack[]> {
    const [rows] = await this.pool.query<TrackRow[]>(this.scope ? 'SELECT * FROM exploration_tracks WHERE deleted_at IS NULL AND owner_user_id=? ORDER BY normalized_name ASC,id ASC' : 'SELECT * FROM exploration_tracks WHERE deleted_at IS NULL ORDER BY normalized_name ASC,id ASC', this.scope ? [this.scope.userId] : [])
    return rows.map(mapTrack)
  }

  async listDeleted(): Promise<DeletedExplorationTrackListEntry[]> {
    const [rows] = await this.pool.query<TrackRow[]>(this.scope ? 'SELECT * FROM exploration_tracks WHERE deleted_at IS NOT NULL AND owner_user_id=? ORDER BY deleted_at DESC,id ASC' : 'SELECT * FROM exploration_tracks WHERE deleted_at IS NOT NULL ORDER BY deleted_at DESC,id ASC', this.scope ? [this.scope.userId] : [])
  ...
  async getHistory(id: string): Promise<ExplorationTrackHistory | undefined> {
    ...
    const [rows] = await this.pool.query<ItemRow[]>(this.scope ? 'SELECT * FROM items WHERE exploration_track_id=? AND deleted_at IS NULL AND owner_user_id=? ORDER BY created_at DESC,id ASC' : 'SELECT * FROM items WHERE exploration_track_id=? AND deleted_at IS NULL ORDER BY created_at DESC,id ASC', this.scope ? [id,this.scope.userId] : [id])
  ...
  async getItemContext(itemId: string): Promise<ItemExplorationTrackContext | undefined> {
    const [items] = await this.pool.query<ItemRow[]>(this.scope ? 'SELECT * FROM items WHERE id=? AND owner_user_id=?' : 'SELECT * FROM items WHERE id=?', this.scope ? [itemId,this.scope.userId] : [itemId])
    const item = items[0]
  ...
  async listItemsByTrackAndStatus(trackId: string, status: CurrentAssociatedStatus): Promise<Item[]> {
    ...
    if (this.scope && !(await this.getById(trackId))) throw new ExplorationTrackError('探索主线不存在', 'not-found')
    const [rows] = await this.pool.query<ItemRow[]>(this.scope ? 'SELECT * FROM items WHERE exploration_track_id=? AND status=? AND deleted_at IS NULL AND owner_user_id=? ORDER BY updated_at DESC,id ASC' : 'SELECT * FROM items WHERE exploration_track_id=? AND status=? AND deleted_at IS NULL ORDER BY updated_at DESC,id ASC', this.scope ? [trackId,status,this.scope.userId] : [trackId,status])
  ...
  private async historyEntries(rows: ItemRow[]): Promise<ExplorationTrackItem[]> {
    ...
    for (const row of rows) {
      ...
      const [reviews] = await this.pool.query<ReviewRow[]>(this.scope ? 'SELECT item_id,actual_action,result FROM reviews WHERE item_id=? AND owner_user_id=?' : 'SELECT item_id,actual_action,result FROM reviews WHERE item_id=?', this.scope ? [item.id,this.scope.userId] : [item.id])
  ...
  private async lockTrack(connection: PoolConnection, id: string): Promise<ExplorationTrack | undefined> {
    const [rows] = await connection.query<TrackRow[]>(this.scope ? 'SELECT * FROM exploration_tracks WHERE id=? AND owner_user_id=? FOR UPDATE' : 'SELECT * FROM exploration_tracks WHERE id=? FOR UPDATE', this.scope ? [id,this.scope.userId] : [id])
    return rows[0] && mapTrack(rows[0])
  }
  private async lockActiveTrack(connection: PoolConnection, id: string): Promise<ExplorationTrack | undefined> {
    const [rows] = await connection.query<TrackRow[]>(this.scope ? 'SELECT * FROM exploration_tracks WHERE id=? AND deleted_at IS NULL AND owner_user_id=? FOR UPDATE' : 'SELECT * FROM exploration_tracks WHERE id=? AND deleted_at IS NULL FOR UPDATE', this.scope ? [id, this.scope.userId] : [id])
    return rows[0] && mapTrack(rows[0])
  }
  private async lockActiveItem(connection: PoolConnection, id: string): Promise<ItemRow | undefined> {
    const [rows] = await connection.query<ItemRow[]>(this.scope ? 'SELECT * FROM items WHERE id=? AND deleted_at IS NULL AND owner_user_id=? FOR UPDATE' : 'SELECT * FROM items WHERE id=? AND deleted_at IS NULL FOR UPDATE', this.scope ? [id,this.scope.userId] : [id])
    return rows[0]
  }
  private async lockTracksOrdered(connection: PoolConnection, ids: Array<string | null>): Promise<Map<string, ExplorationTrack>> {
    ...
    const [rows] = await connection.query<TrackRow[]>(this.scope ? `SELECT * FROM exploration_tracks WHERE id IN (${placeholders}) AND owner_user_id=? ORDER BY id ASC FOR UPDATE` : `SELECT * FROM exploration_tracks WHERE id IN (${placeholders}) ORDER BY id ASC FOR UPDATE`, this.scope ? [...uniqueIds, this.scope.userId] : uniqueIds)
