import type { DashboardRepository, DashboardSnapshot, Item, ItemStatus, ItemStatusEvent, Method, MethodApplication, MethodEvidence, MethodVersion, Review, SearchRepository, SearchResult } from '@knowledge-base/contracts'
  ...
  async getSnapshot(): Promise<DashboardSnapshot> {
    ...
    try {
      ...
      const [items, reviews, methods, methodEvidence, methodVersions, methodApplications, itemStatusEvents] = await Promise.all([
        connection.query<ItemRow[]>('SELECT * FROM items WHERE deleted_at IS NULL ORDER BY id ASC'),
        connection.query<ReviewRow[]>('SELECT * FROM reviews ORDER BY id ASC'),
        connection.query<MethodRow[]>('SELECT * FROM methods WHERE deleted_at IS NULL ORDER BY id ASC'),
        connection.query<EvidenceRow[]>('SELECT * FROM method_evidence ORDER BY id ASC'),
        connection.query<VersionRow[]>('SELECT * FROM method_versions ORDER BY method_id ASC,version ASC,id ASC'),
        connection.query<ApplicationRow[]>('SELECT * FROM method_applications ORDER BY id ASC'),
        connection.query<EventRow[]>('SELECT * FROM item_status_events ORDER BY item_id ASC,created_at ASC,id ASC'),
      ])
...
export class MySqlSearchRepository implements SearchRepository {
  constructor(private readonly pool: Pool, private readonly hooks?: MySqlSearchRepositoryTestHooks) {}
  ...
  async search(query: string): Promise<SearchResult[]> {
    ...
    try {
      ...
      await connection.query('START TRANSACTION WITH CONSISTENT SNAPSHOT')
      started = true
      const [items] = await connection.query<ItemRow[]>('SELECT * FROM items WHERE deleted_at IS NULL ORDER BY id ASC')
      await this.hooks?.afterItemsRead?.()
      const [reviews] = await connection.query<ReviewRow[]>('SELECT * FROM reviews ORDER BY id ASC')
      const [methods] = await connection.query<MethodRow[]>('SELECT * FROM methods WHERE deleted_at IS NULL ORDER BY id ASC')
      const [versions] = await connection.query<VersionRow[]>('SELECT * FROM method_versions ORDER BY method_id ASC,version ASC,id ASC')
      await connection.commit()
      started = false
