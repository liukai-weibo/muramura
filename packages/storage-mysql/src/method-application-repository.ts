...
export class MySqlMethodApplicationRepository implements MethodApplicationRepository {
  ...
  async createItem(input: CreateMethodApplicationInput): Promise<Item> {
    ...
    return runInMySqlTransaction(this.pool, async connection => {
      ...
      try {
        await connection.execute(this.scope ? 'INSERT INTO method_applications(id,method_id,method_version,item_id,created_at,updated_at,owner_user_id) VALUES(?,?,?,?,?,?,?)' : 'INSERT INTO method_applications(id,method_id,method_version,item_id,created_at,updated_at) VALUES(?,?,?,?,?,?)', this.scope ? [createId(), method.id, method.version, item.id, mysqlDateTime(createdAt), mysqlDateTime(createdAt), this.scope.userId] : [createId(), method.id, method.version, item.id, mysqlDateTime(createdAt), mysqlDateTime(createdAt)])
      } catch (error) {
