...
function TrackItem({ item, onOpen }: { item: ExplorationTrackItem; onOpen: (locator: ItemLocator) => void }) {
  return <View className='exploration-track-item'>
    <View className='exploration-track-item-heading'>
      <View className='exploration-track-item-copy'><Text className='exploration-status'>{statusLabels[item.item.status] ?? item.item.status}</Text><Text className='exploration-track-item-title'>{item.item.title}</Text></View>
      <Button className='exploration-inline-button' onClick={() => onOpen(item.locator)}>查看</Button>
    </View>
    {item.item.startAction && <Text className='exploration-track-item-meta'>开始时准备：{item.item.startAction}</Text>}
    {item.item.status === 'paused' && <Text className='exploration-track-item-meta'>当前暂时停止投入</Text>}
    {(item.item.status === 'reviewed' || item.item.status === 'abandoned') && <Summary item={item} />}
  </View>
}

export function ExplorationPrototype({ onOpenItem, onOpenItems }: { onOpenItem: (locator: ItemLocator) => void; onOpenItems: (status: CurrentAssociatedStatus, items: import('@knowledge-base/contracts').Item[]) => void }) {
  ...
  const createTrack = async () => {
    ...
    try { const track = await apiClient.createExplorationTrack(newName); setNewName(''); await loadList(false, track.id) }
  ...
  const capture = async (saveForLater: boolean) => {
    ...
    setCreating(true); setError('')
    try { await apiClient.createIdea({ title: draft, saveForLater, explorationTrack: { type: 'existing', trackId: history.track.id } }); setDraft(captureDraftAfterWrite(draft, true)); await loadList(false, history.track.id) }
    catch (cause) { setDraft(captureDraftAfterWrite(draft, false)); preserveUnknownOutcome(cause, '创建未完成，请重试。') }
    finally { setCreating(false) }
  }
  const rename = async () => {
  ...
  const remove = async () => {
    ...
    catch (cause) { preserveUnknownOutcome(cause, '移入已删除主线未完成，请重试。') }
    finally { setCreating(false) }
  }
  const restore = async () => {
    if (!history) return
  ...
  return <View className='exploration-prototype module-panel'>
    <View className='exploration-prototype-header'><View><Text className='section-kicker'>探索主线</Text><Text className='exploration-prototype-title'>让独立行动形成可回看的过程</Text><Text className='module-description'>主线、关联事项与历史均来自本地 loopback API。</Text></View>
      {!showDeleted && <View className='exploration-create'><Input value={newName} onInput={(event) => setNewName(event.detail.value)} placeholder='新建探索主线名称' /><Button className='primary-button' disabled={creating || unknownOutcome || !newName.trim()} onClick={createTrack}>新建</Button></View>}</View>
    ...
    <View className='exploration-workspace'>
      <View className='exploration-list-panel'>
        <View className='exploration-list-heading'><View><Text className='section-kicker'>{showDeleted ? '已删除主线' : '探索主线'}</Text><Text>{showDeleted ? '只读回看' : '定位一段长期行动历程'}</Text></View></View>
      ...
      <View className='exploration-detail-panel'>
        {detailLoading ? <View className='exploration-state'><Text>正在载入探索历史…</Text></View> : !history && listState === 'error' ? <View className='exploration-state'><Text>暂时无法载入探索主线与探索历史。</Text></View> : !history ? <View className='exploration-state'><Text>{showDeleted ? '选择一条已删除主线，查看保留的历史事实。' : '选择一条探索主线，查看它串联的独立行动与复盘事实。'}</Text></View> : <View className={`exploration-detail ${deleted ? 'deleted' : ''}`}>
          ...
          {!deleted && <View className='exploration-section exploration-capture'><Text className='exploration-section-title'>在「{history.track.name}」下记下想做的事</Text><Input className='exploration-capture-input' value={draft} onInput={(event) => setDraft(event.detail.value)} placeholder='例如：预约一次线下二胡体验课' disabled={creating} /><Text className='exploration-capture-meta'>自动归入：{history.track.name}</Text><View className='exploration-capture-actions'><Button className='primary-button' disabled={creating || unknownOutcome || !draft.trim()} onClick={() => capture(false)}>加入想试试</Button><Button className='secondary-button' disabled={creating || unknownOutcome || !draft.trim()} onClick={() => capture(true)}>加入以后再说</Button></View></View>}
    ...
    {confirmDelete && <View className='exploration-confirm'><Text>移入已删除主线？关联事项、复盘和历史事实不会被删除或解除关联。</Text><View><Button className='secondary-button' disabled={creating} onClick={() => setConfirmDelete(false)}>取消</Button><Button className='primary-button' disabled={creating || unknownOutcome} onClick={remove}>移入已删除主线</Button></View></View>}
