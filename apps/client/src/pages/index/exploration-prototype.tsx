...
export function ExplorationPrototype({ explorationFactsVersion, restoreFactsVersion, onRestoreFactsConfirmed, onRestoreFactsFailed, onExplorationTrackCountChange, onItemsChanged, onRefresh, showManualRefresh = true, onOpenItem, onOpenItems, itemUpdatedAtById, paneWidth, onPaneResizeStart }: { explorationFactsVersion: number; restoreFactsVersion: number; onRestoreFactsConfirmed: () => void; onRestoreFactsFailed: (message: string) => void; onExplorationTrackCountChange: (count: number) => void; onItemsChanged: () => Promise<void>; onRefresh?: () => void | Promise<void>; showManualRefresh?: boolean; onOpenItem: (locator: ItemLocator) => void; onOpenItems: (status: CurrentAssociatedStatus, items: import('@knowledge-base/contracts').Item[]) => void; itemUpdatedAtById: ReadonlyMap<string, string>; paneWidth?: number; onPaneResizeStart?: (event: React.PointerEvent) => void }) {
  ...
  return <ItemUpdatedAtContext.Provider value={itemUpdatedAtById}><View ref={explorationRootRef} className='exploration-prototype module-panel'>
    <View className='exploration-prototype-header'><View><Text className='section-kicker'>长期探索</Text><Text className='exploration-prototype-title'>让独立行动形成可回看的过程</Text><Text className='module-description'>长期探索、关联事项与历史均来自本地 loopback API。</Text></View>
      <Button className='primary-button exploration-create-trigger' disabled={creating || unknownOutcome} onClick={() => setCreateDialogOpen(true)}>新建长期探索</Button></View>
    ...
    <View className='exploration-workspace' style={paneWidth ? { gridTemplateColumns: `${paneWidth}px minmax(0, 1fr)` } : undefined}>
      <View className='exploration-list-panel'>
        {onRefresh && showManualRefresh && <Button className='exploration-inline-button list-refresh-button exploration-list-refresh' disabled={listLoading} onClick={() => void Promise.resolve(onRefresh()).catch((cause: unknown) => setError(messageOf(cause, '刷新长期探索失败')))}>刷新</Button>}
        <View className='exploration-list-heading'><View><Text className='section-kicker'>长期探索</Text><Text>记录一段长期兴趣与历程</Text></View>{listLoading && listReadSucceeded && <Text className='exploration-refreshing'>正在更新…</Text>}</View>
      ...
      <div className='desktop-pane-divider' style={paneWidth ? { left: `${paneWidth}px` } : undefined} role='separator' aria-label='调整列表栏宽度' onPointerDown={onPaneResizeStart} />
