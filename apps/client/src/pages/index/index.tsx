...
import { DailyDietDetailModal } from './features/daily-diet/daily-diet-detail-modal'
import { HomeAiCardEditorModal } from './features/home-ai-card/home-ai-card-editor-modal'
import { HomeAiCardDetailModal } from './features/home-ai-card/home-ai-card-detail-modal'
import { autoGenerateDailySummaryIfNeeded, generateDailySummaryNow, resetAutoGenerateGuard, startMidnightAutoGenerate } from './features/daily-summary/daily-summary-auto'
import { autoGenerateDailyDietIfNeeded, generateDailyDietNow, resetDailyDietAutoGenerateGuard } from './features/daily-diet/daily-diet-auto'
import { autoGenerateHomeAiCardsIfNeeded, resetHomeAiCardsAutoGenerateGuard } from './features/home-ai-card/home-ai-cards-auto'
import './features/daily-summary/daily-summary.scss'
...
type ContentModule = 'actions' | 'explorations' | 'methods' | 'insights' | 'ai' | 'settings' | 'administration' | 'aiConfiguration' | 'audit'
type PrimaryModule = 'home' | 'workbench' | 'dailyNotes' | 'ai' | 'me'
type WorkbenchTab = 'actions' | 'explorations' | 'methods' | 'mood' | 'meals'
type MyTab = 'profile' | 'insights' | 'storage' | 'administration' | 'aiConfiguration' | 'audit'
...
function AuthenticatedWorkspace({ session, logoutBusy, logoutUnknownOutcome, logoutError, onLogout, onConfirmLogoutOutcome, onPasswordChanged, colorTheme, onToggleColorTheme }: AuthenticatedWorkspaceProps) {
  ...
  const [homeDailySummaryOpen, setHomeDailySummaryOpen] = useState(false)
  ...
  const [summaryChangedAt, setSummaryChangedAt] = useState(0)
  // 页面级生成任务：弹窗关闭后仍在后台生成，卡片展示流式预览；draft 未落库前优先显示
  const summaryAbortRef = useRef<AbortController>()
  const [todaySummaryGenerating, setTodaySummaryGenerating] = useState(false)
  const [todaySummaryDraft, setTodaySummaryDraft] = useState<string>()
  const [todaySummaryAiUnavailable, setTodaySummaryAiUnavailable] = useState(false)
  const [homeDailyDietOpen, setHomeDailyDietOpen] = useState(false)
  const [todayDiet, setTodayDiet] = useState<DailyDietRecommendation | undefined>()
  const [todayDietLoading, setTodayDietLoading] = useState(false)
  const [dietChangedAt, setDietChangedAt] = useState(0)
  const dietAbortRef = useRef<AbortController>()
  const [todayDietGenerating, setTodayDietGenerating] = useState(false)
  const [todayDietDraft, setTodayDietDraft] = useState<string>()
  const [todayDietAiUnavailable, setTodayDietAiUnavailable] = useState(false)
  const [homeAiCards, setHomeAiCards] = useState<HomeAiCard[]>([])
  const [homeAiCardPreviews, setHomeAiCardPreviews] = useState<Record<string, string>>({})
  const [homeAiCardsLoading, setHomeAiCardsLoading] = useState(false)
  const [homeAiCardsFailed, setHomeAiCardsFailed] = useState(false)
  const [homeAiCardsChangedAt, setHomeAiCardsChangedAt] = useState(0)
  const [homeAiCardEditor, setHomeAiCardEditor] = useState<{ card?: HomeAiCard }>()
  const [homeAiCardEditorSaving, setHomeAiCardEditorSaving] = useState(false)
  const [homeAiCardEditorError, setHomeAiCardEditorError] = useState('')
  const [homeAiCardDetail, setHomeAiCardDetail] = useState<HomeAiCard>()
  const [homeAiCardDetailContent, setHomeAiCardDetailContent] = useState<string>()
  const [homeAiCardDetailLoading, setHomeAiCardDetailLoading] = useState(false)
  const [homeAiCardDetailGenerating, setHomeAiCardDetailGenerating] = useState(false)
  const [homeAiCardDetailError, setHomeAiCardDetailError] = useState('')
  const [homeAiCardDeleteConfirm, setHomeAiCardDeleteConfirm] = useState<HomeAiCard>()
  ...
  useEffect(() => {
    ...
    const loadToday = async () => {
      ...
      try {
        let summary = await apiClient.getDailySummary(todayLocalDate())
        if (active && !summary) {
          await autoGenerateDailySummaryIfNeeded((text) => { if (active) setTodaySummaryDraft(text) })
          if (!active) return
          summary = await apiClient.getDailySummary(todayLocalDate())
          if (active) setTodaySummaryDraft(undefined)
        }
        if (active) setTodaySummary(summary ?? undefined)
      } catch {
        // 保持卡片空态即可
      } finally {
        if (active) setTodaySummaryLoading(false)
      }
    }
    void loadToday()
    const stopMidnight = startMidnightAutoGenerate(() => { void loadToday() })
    return () => { active = false; stopMidnight() }
  }, [summaryChangedAt])

  useEffect(() => {
    let active = true
    const loadDietToday = async () => {
      setTodayDietLoading(true)
      try {
        let dietRec = await apiClient.getDailyDietRecommendation(todayLocalDate())
        if (active && !dietRec) {
          await autoGenerateDailyDietIfNeeded((text) => { if (active) setTodayDietDraft(text) })
          if (!active) return
          dietRec = await apiClient.getDailyDietRecommendation(todayLocalDate())
          if (active) setTodayDietDraft(undefined)
        }
        if (active) setTodayDiet(dietRec ?? undefined)
      } catch {
        // 保持卡片空态即可
      } finally {
        if (active) setTodayDietLoading(false)
      }
    }
    void loadDietToday()
    const stopMidnight = startMidnightAutoGenerate(() => { void loadDietToday() })
    return () => { active = false; stopMidnight() }
  }, [dietChangedAt])

  // —— 页面级手动生成：弹窗关闭后仍在后台持续生成，卡片与弹窗共享流式 draft ——
  const startSummaryGeneration = useCallback(async (force: boolean) => {
    if (todaySummaryGenerating) return
    if (!force && summaryAbortRef.current && !summaryAbortRef.current.signal.aborted) return
    const controller = new AbortController()
    summaryAbortRef.current = controller
    setTodaySummaryGenerating(true)
    setTodaySummaryAiUnavailable(false)
    setTodaySummaryDraft('')
    try {
      const result = await generateDailySummaryNow({ signal: controller.signal, onToken: (text) => { if (!controller.signal.aborted) setTodaySummaryDraft(text) } })
      if (!result.generated) {
        if (result.aiUnavailable) setTodaySummaryAiUnavailable(true)
        if (result.failed) setTodaySummaryDraft('')
      }
      if (result.generated) {
        setTodaySummaryDraft(undefined)
        setSummaryChangedAt((value) => value + 1)
      }
    } finally {
      if (summaryAbortRef.current === controller) summaryAbortRef.current = undefined
      setTodaySummaryGenerating(false)
    }
  }, [todaySummaryGenerating])

  const startDietGeneration = useCallback(async (force: boolean) => {
    if (todayDietGenerating) return
    if (!force && dietAbortRef.current && !dietAbortRef.current.signal.aborted) return
    const controller = new AbortController()
    dietAbortRef.current = controller
    setTodayDietGenerating(true)
    setTodayDietAiUnavailable(false)
    setTodayDietDraft('')
    try {
      const result = await generateDailyDietNow({ signal: controller.signal, onToken: (text) => { if (!controller.signal.aborted) setTodayDietDraft(text) } })
      if (!result.generated) {
        if (result.aiUnavailable) setTodayDietAiUnavailable(true)
        if (result.failed) setTodayDietDraft('')
      }
      if (result.generated) {
        setTodayDietDraft(undefined)
        setDietChangedAt((value) => value + 1)
      }
    } finally {
      if (dietAbortRef.current === controller) dietAbortRef.current = undefined
      setTodayDietGenerating(false)
    }
  }, [todayDietGenerating])

  useEffect(() => {
    let active = true
    const loadHomeAiCards = async () => {
      setHomeAiCardsLoading(true)
      setHomeAiCardsFailed(false)
      try {
        const [cards, caches] = await Promise.all([
          apiClient.listHomeAiCards(),
          apiClient.listHomeAiCardCaches(todayLocalDate()),
        ])
        if (!active) return
        setHomeAiCards(cards)
        const previews: Record<string, string> = {}
        for (const cache of caches) previews[cache.cardId] = cache.aiOutput
        setHomeAiCardPreviews(previews)
        const dailyWithoutCache = cards.filter(card => card.refreshMode === 'daily' && !card.isHidden && !caches.some(cache => cache.cardId === card.id))
        if (dailyWithoutCache.length > 0) {
          await autoGenerateHomeAiCardsIfNeeded()
          if (!active) return
          const nextCaches = await apiClient.listHomeAiCardCaches(todayLocalDate())
          const nextPreviews: Record<string, string> = {}
          for (const cache of nextCaches) nextPreviews[cache.cardId] = cache.aiOutput
          setHomeAiCardPreviews(nextPreviews)
          const stillMissing = cards.filter(card => card.refreshMode === 'daily' && !card.isHidden && !nextCaches.some(cache => cache.cardId === card.id))
          if (stillMissing.length > 0) setHomeAiCardsFailed(true)
        }
      } catch {
        // 保持卡片空态即可
      } finally {
        if (active) setHomeAiCardsLoading(false)
      }
    }
    void loadHomeAiCards()
    const stopMidnight = startMidnightAutoGenerate(() => { void loadHomeAiCards() })
    return () => { active = false; stopMidnight() }
  }, [homeAiCardsChangedAt])

  useEffect(() => () => resetHomeAiCardsAutoGenerateGuard(), [])
  useEffect(() => () => resetDailyDietAutoGenerateGuard(), [])
  ...
  useEffect(() => {
    ...
    if (activeModule === 'administration' || activeModule === 'aiConfiguration' || activeModule === 'audit') setAdministrationMounted(true)
  ...
  useEffect(() => {
    ...
    } else if (activeModule === 'administration' || activeModule === 'aiConfiguration' || activeModule === 'audit') {
      setPrimaryModule('me')
  ...
  const openPrimaryModule = (target: PrimaryModule) => {
    ...
    requestLeaveAllDrafts(() => {
      ...
      else if (target === 'me' && (myTab === 'administration' || myTab === 'aiConfiguration' || myTab === 'audit')) setActiveModule(myTab)
    })
  }
  ...
  const openMyTab = (tab: MyTab) => {
    requestLeaveAllDrafts(() => {
      ...
      else if (tab === 'administration' || tab === 'aiConfiguration' || tab === 'audit') setActiveModule(tab)
    })
  }
    ...
    <View className='app-shell' data-color-theme={colorTheme} data-display-effect={displayEffectMode}>
      ...
      <View className='app-main'>
        ...
        {primaryModule === 'home' && <HomeDashboard
          ...
          dailyDiet={todayDiet}
          dailyDietLoading={todayDietLoading}
          dailyDietStreamPreview={todayDietDraft}
          homeAiCards={homeAiCards}
          homeAiCardPreviews={homeAiCardPreviews}
          homeAiCardLoading={homeAiCardsLoading}
          homeAiCardFailed={homeAiCardsFailed}
          onOpenHomeAiCard={(card) => { setHomeAiCardDetail(card); setHomeAiCardDetailContent(homeAiCardPreviews[card.id]); setHomeAiCardDetailLoading(false); setHomeAiCardDetailError('') }}
          onEditHomeAiCard={(card) => { setHomeAiCardEditor({ card }); setHomeAiCardEditorError('') }}
          onAddHomeAiCard={() => { setHomeAiCardEditor({}); setHomeAiCardEditorError('') }}
        />}
        ...
        {homeDailySummaryOpen && <DailySummaryDetailModal initialDate={todayLocalDate()} generating={todaySummaryGenerating} draft={todaySummaryDraft} aiUnavailable={todaySummaryAiUnavailable} onGenerate={() => { void startSummaryGeneration(true) }} onClose={() => setHomeDailySummaryOpen(false)} onChanged={() => { setSummaryChangedAt((v) => v + 1) }} />}
        {homeDailyDietOpen && <DailyDietDetailModal initialDate={todayLocalDate()} generating={todayDietGenerating} draft={todayDietDraft} aiUnavailable={todayDietAiUnavailable} onGenerate={() => { void startDietGeneration(true) }} onClose={() => setHomeDailyDietOpen(false)} onChanged={() => { setDietChangedAt((v) => v + 1) }} />}

        {homeAiCardEditor && <HomeAiCardEditorModal
          card={homeAiCardEditor.card}
          saving={homeAiCardEditorSaving}
          error={homeAiCardEditorError}
          onClose={() => { if (!homeAiCardEditorSaving) setHomeAiCardEditor(undefined) }}
          onSave={async (input) => {
            setHomeAiCardEditorSaving(true)
            setHomeAiCardEditorError('')
            try {
              if (homeAiCardEditor.card) await apiClient.updateHomeAiCard(homeAiCardEditor.card.id, input)
              else await apiClient.createHomeAiCard(input)
              setHomeAiCardEditor(undefined)
              setHomeAiCardsChangedAt((v) => v + 1)
            } catch (cause: unknown) {
              setHomeAiCardEditorError(cause instanceof Error ? cause.message : '保存失败')
            } finally {
              setHomeAiCardEditorSaving(false)
            }
          }}
        />}

        {homeAiCardDetail && <HomeAiCardDetailModal
          card={homeAiCardDetail}
          content={homeAiCardDetailContent}
          loading={homeAiCardDetailLoading}
          generating={homeAiCardDetailGenerating}
          error={homeAiCardDetailError}
          onClose={() => { if (!homeAiCardDetailGenerating) { setHomeAiCardDetail(undefined); setHomeAiCardDetailContent(undefined) } }}
          onDelete={() => setHomeAiCardDeleteConfirm(homeAiCardDetail)}
          onRefresh={async () => {
            if (homeAiCardDetailGenerating) return
            setHomeAiCardDetailGenerating(true)
            setHomeAiCardDetailError('')
            let output = ''
            const controller = new AbortController()
            try {
              const status = await apiClient.getAiConfigStatus()
              if (!status.configured) { setHomeAiCardDetailError('AI 尚未配置，请在「我 → AI 参数」中配置后重试。'); return }
              for await (const event of apiClient.streamExperimentalAiChatEphemeral([{ role: 'user', content: homeAiCardDetail.aiPrompt }], controller.signal)) {
                if (event.type === 'token') output += event.content
                if (event.type === 'incomplete' || event.type === 'error') { setHomeAiCardDetailError('生成未完成，请稍后重试'); return }
              }
              if (!output.trim()) { setHomeAiCardDetailError('生成结果为空，请稍后重试'); return }
              await apiClient.upsertHomeAiCardCache(homeAiCardDetail.id, todayLocalDate(), output.trim())
              setHomeAiCardDetailContent(output.trim())
              setHomeAiCardsChangedAt((v) => v + 1)
            } catch (cause: unknown) {
              if (isApiClientAbort(cause) || (cause instanceof Error && cause.name === 'AbortError')) return
              setHomeAiCardDetailError(cause instanceof Error ? cause.message : '生成失败')
            } finally {
              setHomeAiCardDetailGenerating(false)
            }
          }}
        />}

        {homeAiCardDeleteConfirm && <View className='home-ai-card-delete-confirm-backdrop' role='dialog' aria-modal='true' aria-label='删除AI卡片' onClick={(event) => { if (event.target === event.currentTarget) setHomeAiCardDeleteConfirm(undefined) }}>
          <View className='home-ai-card-delete-confirm-card'>
            <Text className='home-ai-card-delete-confirm-title'>删除「{homeAiCardDeleteConfirm.cardTitle}」？</Text>
            <Text className='home-ai-card-delete-confirm-hint'>卡片及其历史生成内容将一并删除，不可恢复。</Text>
            <View className='home-ai-card-delete-confirm-actions'>
              <Button className='home-ai-card-editor-cancel' onClick={() => setHomeAiCardDeleteConfirm(undefined)}>取消</Button>
              <Button className='home-ai-card-detail-delete' onClick={async () => {
                const card = homeAiCardDeleteConfirm
                setHomeAiCardDeleteConfirm(undefined)
                try {
                  await apiClient.deleteHomeAiCard(card.id)
                  setHomeAiCardDetail(undefined)
                  setHomeAiCardDetailContent(undefined)
                  setHomeAiCardEditor(undefined)
                  setHomeAiCardsChangedAt((v) => v + 1)
                } catch (cause: unknown) {
                  setHomeAiCardDetailError(cause instanceof Error ? cause.message : '删除失败')
                }
              }}>删除</Button>
            </View>
          </View>
      ...
      {isPlatformAdministrator && !managementAccessDenied && (primaryModule === 'me' && myTab === 'audit' || administrationMounted) && <AuditCenter
        authenticationContext={`${session.user.id}-${session.user.createdAt}`}
        visible={primaryModule === 'me' && myTab === 'audit'}
      />}
