...
import { autoGenerateDailySummaryIfNeeded, resetAutoGenerateGuard, startMidnightAutoGenerate } from './features/daily-summary/daily-summary-auto'
import { autoGenerateDailyDietIfNeeded, resetDailyDietAutoGenerateGuard } from './features/daily-diet/daily-diet-auto'
  ...
  useEffect(() => {
    ...
    const loadToday = async () => {
      ...
      try {
        ...
        if (active && !summary) {
          await autoGenerateDailySummaryIfNeeded()
          if (!active) return
  ...
  useEffect(() => {
    ...
    const loadDietToday = async () => {
      ...
      try {
        ...
        if (active && !dietRec) {
          await autoGenerateDailyDietIfNeeded()
          if (!active) return
  ...
  return (
    <View className='app-shell' data-color-theme={colorTheme} data-display-effect={displayEffectMode}>
      ...
      <View className='app-main'>
        ...
        {homeDailySummaryOpen && <DailySummaryDetailModal initialDate={todayLocalDate()} onClose={() => setHomeDailySummaryOpen(false)} onChanged={() => { setSummaryChangedAt((v) => v + 1) }} />}
        {homeDailyDietOpen && <DailyDietDetailModal initialDate={todayLocalDate()} onClose={() => setHomeDailyDietOpen(false)} onChanged={() => { setDietChangedAt((v) => v + 1) }} />}
