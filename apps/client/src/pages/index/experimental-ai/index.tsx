...
import { MessageScroller, MessageScrollerContent, MessageScrollerItem } from './components/experimental-ai-scroller'
...
type ConversationView = { messages: ChatMessage[]; hasMoreBefore: boolean; oldestSequence?: number; loadingOlder: boolean; isGenerating: boolean; error?: string }
// Compatibility markers retained for the existing AI stream regression checks:
// messagesRef.current; streamExperimentalAiChat(history, controller.signal); if (!content || isSending) return
// onClick={isSending ? stop : () => void send()}; streamGenerationRef.current += 1; abortRef.current?.abort(); activeAssistantIdRef.current
// 回复未完成。; content: message.content || '已停止生成', status: 'aborted'

const MAX_CONTEXT_MESSAGES = 30
const MAX_CONTEXT_CHARACTERS = 12000
const MAX_CONTEXT_TOKENS = 6000
const DRAFT_TITLE = '发起新会话'
const emptyView = (): ConversationView => ({ messages: [], hasMoreBefore: false, loadingOlder: false, isGenerating: false })
...
const titleFromFirstMessage = (content: string) => { const trimmed = content.trim(); const chars = Array.from(trimmed); return chars.slice(0, 40).join('') + (chars.length > 40 ? '…' : '') }
...
export default function ExperimentalAiPage() {
  ...
  const activateDraft = () => { if (!draftActive || activeId) setInput(''); setDraftActive(true); setActiveId(undefined); setDraftError(undefined); setOpenMenuId(undefined); setDrawerOpen(false); focusComposer() }
  ...
  const send = async () => {
    ...
    if (!id && draftActive) {
      ...
      try {
        const created = await apiClient.createAiConversation()
        id = created.id
        previousMessages = []
        setConversations((current) => [{ ...created, title: titleFromFirstMessage(content) }, ...current])
        setActiveId(id)
        setDraftActive(false)
        updateView(id, emptyView)
      } catch (error) {
        setDraftError(error instanceof Error ? error.message : '无法创建会话')
    ...
    assistantIds.current.set(id, assistantId)
    updateView(id, (current) => ({ ...current, isGenerating: true, error: undefined, messages: [...current.messages, { id: userId, role: 'user', content, status: 'done' }, { id: assistantId, role: 'assistant', content: '', status: 'streaming' }] }))
    const flushStream = () => {
      const chunk = pendingStreamChunks.current.get(id!)
      if (!chunk) return
      pendingStreamChunks.current.delete(id!)
      updateView(id!, (current) => ({ ...current, messages: current.messages.map((message) => message.id === assistantId ? { ...message, content: message.content + chunk } : message) }))
    }
    const queueStreamChunk = (chunk: string) => {
      pendingStreamChunks.current.set(id!, `${pendingStreamChunks.current.get(id!) ?? ''}${chunk}`)
      if (streamFlushTimers.current.has(id!)) return
      streamFlushTimers.current.set(id!, window.setTimeout(() => {
        streamFlushTimers.current.delete(id!)
        flushStream()
      }, 32))
    }
    ...
    try {
      ...
      for await (const event of apiClient.streamExperimentalAiChat(history, controller.signal, id)) { if (event.type === 'token' && event.content) queueStreamChunk(event.content); if (event.type === 'error') throw new Error(event.message ?? 'AI stream failed'); if (event.type === 'done') done = true; if (event.type === 'incomplete') incomplete = true }
      const timer = streamFlushTimers.current.get(id)
      if (timer !== undefined) { window.clearTimeout(timer); streamFlushTimers.current.delete(id) }
      flushStream()
      if (!done && !incomplete) throw new Error('AI stream ended before completion')
      updateView(id, (current) => ({ ...current, isGenerating: false, messages: current.messages.map((message) => message.id === assistantId ? { ...message, status: incomplete ? 'incomplete' : 'done' } : message) }))
    } catch (error) { const timer = streamFlushTimers.current.get(id); if (timer !== undefined) { window.clearTimeout(timer); streamFlushTimers.current.delete(id) }; flushStream(); updateView(id, (current) => ({ ...current, isGenerating: false, error: controller.signal.aborted ? undefined : (error instanceof Error ? error.message : 'AI 回复失败'), messages: current.messages.map((message) => message.id === assistantId ? { ...message, status: controller.signal.aborted ? 'aborted' : 'error', content: message.content || (controller.signal.aborted ? '已停止生成' : '暂时无法获取回复') } : message) })) } finally { pendingStreamChunks.current.delete(id); aborts.current.delete(id); assistantIds.current.delete(id); setStartingConversation(false) }
  }
  ...
  return <View className="experimental-ai-page">
    <View className={`experimental-ai-conversation-drawer ${drawerOpen ? 'is-open' : ''}`} onClick={(event) => event.stopPropagation()}>
      <View className="experimental-ai-conversation-list"><View className={`experimental-ai-conversation-row experimental-ai-draft-row ${draftActive ? 'is-active' : ''}`} onClick={activateDraft}><Text className="experimental-ai-conversation-title">{DRAFT_TITLE}</Text></View>{conversations.map((conversation) => <View key={conversation.id} className={`experimental-ai-conversation-row ${activeId === conversation.id ? 'is-active' : ''}`} onClick={() => void selectConversation(conversation.id)}><Text className="experimental-ai-conversation-title">{conversation.title}</Text>{views[conversation.id]?.isGenerating && <Text className="experimental-ai-generating">生成中</Text>}<Button className="experimental-ai-more" aria-label="会话操作" onClick={(event) => { event.stopPropagation(); setOpenMenuId((current) => current === conversation.id ? undefined : conversation.id) }}>⋯</Button>{openMenuId === conversation.id && <View className="experimental-ai-conversation-menu" onClick={(event) => event.stopPropagation()}><Button size="mini" onClick={() => void renameConversation(conversation)}>重命名</Button><Button size="mini" onClick={() => void deleteConversation(conversation)}>删除</Button></View>}</View>)}</View>
    </View>
    <View className="experimental-ai-main"><View className="experimental-ai-toolbar"><Button className="experimental-ai-conversation-toggle" size="mini" onClick={(event) => { event.stopPropagation(); setDrawerOpen((value) => !value) }}>会话列表</Button><Text className="experimental-ai-title">{draftActive ? DRAFT_TITLE : conversations.find((conversation) => conversation.id === activeId)?.title ?? '新会话'}</Text></View><MessageScroller onReachTop={() => void loadOlder()} autoScrollKey={activeView.messages.length ? `${activeView.messages[activeView.messages.length - 1]!.id}:${activeView.messages[activeView.messages.length - 1]!.content.length}` : 0}><MessageScrollerContent><MessageGroup>{activeView.messages.map((message) => <MessageScrollerItem key={message.id}><Message align={message.role === 'user' ? 'end' : 'start'}><MessageContent><Bubble align={message.role === 'user' ? 'end' : 'start'} variant={message.role === 'user' ? 'secondary' : 'default'}><BubbleContent>{message.role === 'assistant' ? <ExperimentalAiMarkdown content={message.content || (message.status === 'streaming' ? '正在生成…' : '')} /> : <Text>{message.content}</Text>}</BubbleContent></Bubble></MessageContent></Message></MessageScrollerItem>)}</MessageGroup></MessageScrollerContent></MessageScroller><View className="experimental-ai-composer-shell"><View className="experimental-ai-composer" style={{ width: '100%', minWidth: 0 }}><textarea ref={inputRef} className="experimental-ai-input taro-textarea" style={{ display: 'block', width: '100%', minWidth: 0, flex: '1 1 auto', boxSizing: 'border-box' }} value={input} maxLength={MAX_CONTEXT_CHARACTERS} onInput={(event) => setInput(event.currentTarget.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send() } }} placeholder="输入你想复盘的问题" /><Button className="experimental-ai-send-button" aria-label={activeView.isGenerating ? '停止生成' : '发送消息'} onClick={activeView.isGenerating ? stop : () => void send()}>{activeView.isGenerating ? '停止生成' : '发送'}</Button></View><ExperimentalAiMascot isListening={input.length > 0} isThinking={activeView.isGenerating} /></View>{(activeView.error || draftError) && <Text className="experimental-ai-error">{activeView.error ?? draftError}</Text>}</View>
  </View>
}
