...
export default function ExperimentalAiPage() {
  ...
  const send = async () => {
    ...
    try {
      ...
      for await (const event of apiClient.streamExperimentalAiChat(history, controller.signal, id)) { if (event.type === 'token') updateView(id!, (current) => ({ ...current, messages: current.messages.map((message) => message.id === assistantId ? { ...message, content: message.content + (event.content ?? '') } : message) })); if (event.type === 'error') throw new Error(event.message ?? 'AI stream failed'); if (event.type === 'done') done = true; if (event.type === 'incomplete') incomplete = true }
    ...
    } catch (error) { updateView(id, (current) => ({ ...current, isGenerating: false, error: controller.signal.aborted ? undefined : (error instanceof Error ? error.message : 'AI 回复失败'), messages: current.messages.map((message) => message.id === assistantId ? { ...message, status: controller.signal.aborted ? 'aborted' : 'error', content: message.content || (controller.signal.aborted ? '已停止生成' : '暂时无法获取回复') } : message) })) } finally { aborts.current.delete(id); assistantIds.current.delete(id); setStartingConversation(false) }
  }
