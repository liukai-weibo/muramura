...
/** 内部：消费流式接口并落库，返回是否成功写入。 */
async function collectStream(controller: AbortController): Promise<{ ok: boolean; aborted: boolean }> {
  let ok = false
  let aborted = false
  let output = ''
  const today = todayLocalDate()
 ...
 * 首页卡片挂载与午夜定时器回调调用。
 */
export async function autoGenerateDailyDietIfNeeded(): Promise<DailyDietAutoResult> {
  const today = todayLocalDate()
  ...
  const result = await collectStream(controller)
