...
import type { ColorTheme } from '../pages/index/display-effect-preference'
...
export function DesktopTitleBar({ breadcrumb, onSearch, onCapture, colorTheme, onToggleColorTheme, searchContent, workbenchTabs }: DesktopTitleBarProps) {
  ...
  return <View className='desktop-title-bar' data-tauri-drag-region>
    <View className='desktop-title-bar-leading'>
      ...
      {workbenchTabs && <View className='desktop-title-bar-tabs' role='tablist'>
        {workbenchTabs.map((tab) => <button type='button' key={tab.id} className={`desktop-title-bar-tab control-transition ${tab.active ? 'active' : ''}`} role='tab' aria-selected={tab.active} onClick={tab.onClick}>{tab.label}</button>)}
      </View>}
    </View>
    ...
    <View className='desktop-title-bar-actions'>
      ...
      <button type='button' className='desktop-title-bar-capture control-transition' onClick={onCapture}>＋ 快速捕获</button>
      <button type='button' className='desktop-title-bar-theme-toggle control-transition' title={colorTheme === 'light' ? '切换为深色主题' : '切换为浅色主题'} aria-label={colorTheme === 'light' ? '切换为深色主题' : '切换为浅色主题'} onClick={onToggleColorTheme}>{colorTheme === 'light' ? '☾' : '☀'}</button>
