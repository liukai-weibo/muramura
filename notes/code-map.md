...
```text
packages/contracts/src/index.ts            536 行
```
...
读完这 536 行就掌握了系统的全部数据词汇。备份的三个版本 `BackupDocumentV1/V2/V3` 也在这里，V3 比 V2 多 `explorationTracks` 集合；运行时业务读写再由 `CurrentUserScope` 限定当前账户。

### 领域层

```text
packages/domain/src/index.ts               123 行
```
...
apps/api/src/index.ts                      277 行   原生 Node HTTP、认证会话与依赖装配
apps/api/src/api-errors.ts                 147 行   统一错误分类、映射与上报
...
```text
1. packages/contracts/src/index.ts     536 行   认证、归属与业务概念
2. packages/domain/src/index.ts        123 行   认证纯逻辑、业务错误、标题与状态机
3. packages/application/src/index.ts   678 行   认证、初始归属与业务用例
4. apps/api/src/api-errors.ts          147 行   错误如何分类、映射与上报
5. apps/api/src/index.ts               277 行   当前运行的认证与业务接口
